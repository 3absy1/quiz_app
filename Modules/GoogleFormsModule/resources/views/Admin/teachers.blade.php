...
<section class="table-sec pt-3">
    <div class="container px-2 px-md-5">
        <div class="align-items-start border-bottom flex-column">
            <x-googleformsmodule::first-head label="Teacher table" icon="table"/>
                ...
                <div class="modal fade " id="exportModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <form id="exportModalForm"  method="GET" action="{{ route('teachers.selectedExport') }}">
                                @csrf
...
$('#teachers_table').on('page.dt', function() {
    $('.selected-item').text(window.LaravelDataTables['teachers_table'].rows({
        selected: true
    }).count());
    $('.selected-badge').text(window.LaravelDataTables['teachers_table'].rows({
        selected: true
    }).count());
});
    ...
    <label for=""${id}"">  ${$(this).text()}  <label>
        ...
        action: function(e, dt, node, config, cb) {
            ...
            $('#' + id).prop('checked', function(_, oldProp) {
                if (oldProp) {
                    window.LaravelDataTables['teachers_table'].columns(index +
                        2).search(
                        "").draw();
...
function getCheckedCheckboxes() {
    ...
    $(document).on('click','.exportSelected',function() {
        let SelectedRows = JSON.parse(localStorage.getItem('teachers_checkBoxIdsArray'));
