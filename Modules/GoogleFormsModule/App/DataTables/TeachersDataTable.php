...
class TeachersDataTable extends DataTable
{
    ...
    public function html(): HtmlBuilder
    {
        $initCompleteFunction = "function() {
            ...
            window.LaravelDataTables['teachers_table'].columns.adjust().draw();
            window.LaravelDataTables['teachers_table'].on('select deselect',function(){
            var count = window.LaravelDataTables['teachers_table'].rows({ selected: true }).count();
            $('.selected-item').text(count);
            $('.selected-badge').text(count);
            window.LaravelDataTables['teachers_table'].button('copy:name').enable(count > 0);
            window.LaravelDataTables['teachers_table'].button(2).enable(count > 0);
            window.LaravelDataTables['teachers_table'].button('export:name').enable(count > 0);


            // Clear the set before updating
            checkBoxIds.clear();
            // Iterate over each selected row
            window.LaravelDataTables['teachers_table'].rows({ selected: true }).every(function(rowIdx) {
            ...
            window.LaravelDataTables['teachers_table'].rows({ selected: false }).every(function(rowIdx) {
                // Get the corresponding checkbox and check it
                var checkbox = $(this.node()).find('td:eq(0) input[type=\"checkbox\"]');
            ...
            localStorage.setItem('teachers_checkBoxIdsArray', JSON.stringify(checkBoxIdsArray))
                        ...
                        arrOfFilterBtn.push({
                            ...
                            action: function (e, dt, node, config, cb) {
                                ...
                                $('#' + id).prop('checked', function (_, oldProp) {
                                    if (oldProp) {
            window.LaravelDataTables['teachers_table'].columns(index + 1).search("").draw();
                ...
                arrOfFilterBtn.push({
                    ...
                    action: function(e, dt, node, config, cb) {
                        ...
                        $('.filter-modal input[type="text"]').on('input', function() {
                            ...
                            window.LaravelDataTables['teachers_table'].columns(filterIndex).search(searchValue).draw();
                        });
                    }
                });
      ...
      $('.selected-item').text(window.LaravelDataTables['teachers_table'].rows({ selected: true }).count());
      $('.selected-badge').text(window.LaravelDataTables['teachers_table'].rows({ selected: true }).count());
    },lengthMenu: [
        [100,200],
        [100,200]
    ]",
